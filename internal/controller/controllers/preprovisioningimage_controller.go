...
type PreprovisioningImageReconciler struct {
	...
	archToHubIronicAgentImagesMap map[string]string
...
func (r *PreprovisioningImageReconciler) getIronicAgentImageFromHUB(ctx context.Context, log logrus.FieldLogger, infraEnvInternal *common.InfraEnv) string {
	if r.archToHubIronicAgentImagesMap == nil {
		r.archToHubIronicAgentImagesMap = make(map[string]string)
	}
	...
	if len(r.hubReleaseArchitectures) == 0 {
		architectures, archErr := r.OcRelease.GetReleaseArchitecture(log, cv.Status.Desired.Image, r.ReleaseImageMirror, infraEnvInternal.PullSecret)
		if archErr != nil {
			log.WithError(archErr).Warningf("Failed to get release architecture for (%s)", cv.Status.Desired.Image)
			return ""
		}
		r.hubReleaseArchitectures = architectures
		log.Infof("Hub release architectures: %v", architectures)
	}

	if !funk.Contains(r.hubReleaseArchitectures, infraEnvInternal.CPUArchitecture) {
		log.Warningf("Release image architectures %v do not match infraEnv architecture %s", r.hubReleaseArchitectures, infraEnvInternal.CPUArchitecture)
		return ""
	}

	if cachedImage, ok := r.archToHubIronicAgentImagesMap[infraEnvInternal.CPUArchitecture]; ok {
		log.Infof("Using cached ironic agent image (%s) for architecture %s", cachedImage, infraEnvInternal.CPUArchitecture)
		return cachedImage
	}
	...
	r.archToHubIronicAgentImagesMap[infraEnvInternal.CPUArchitecture] = image
	log.Infof("Caching hub ironic agent image %s for architecture %s", image, infraEnvInternal.CPUArchitecture)
...
func (r *PreprovisioningImageReconciler) getIronicAgentImageFromClusterImageSet(ctx context.Context, log logrus.FieldLogger, infraEnvInternal *common.InfraEnv) string {
	...
	if err != nil {
		log.WithError(err).Debugf("Failed to get release image from ClusterImageSet for version %s and arch %s", hubVersion, infraEnvInternal.CPUArchitecture)
		return ""
	}

	if releaseImage == nil || releaseImage.URL == nil || *releaseImage.URL == "" {
		log.Debugf("No release image found in ClusterImageSet for version %s and arch %s", hubVersion, infraEnvInternal.CPUArchitecture)
		return ""
	}
	...
	if err != nil {
		log.WithError(err).Debugf("Failed to extract ironic agent image from release %s", *releaseImage.URL)
		return ""
	}
...
func (r *PreprovisioningImageReconciler) getIronicConfig(ctx context.Context, log logrus.FieldLogger, infraEnv *aiv1beta1.InfraEnv, infraEnvInternal *common.InfraEnv) (*ICCConfig, error) {
	...
	iccIronicAgentImage := iccConfig.IronicAgentImage
	iccConfig.IronicAgentImage = r.getIronicAgentImageByPriority(ctx, log, infraEnv, infraEnvInternal, iccIronicAgentImage)
